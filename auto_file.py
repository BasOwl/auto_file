...
def create_parent_dir() -> None:
    ...
    if not os.path.exists(PATH):  # if the given dir doesn't exist.
        os.makedirs(PATH)
...
def create_file(import_file: list[str]) -> None:
    ...
    for line in import_file:  # repeats for each name in imported_list
        ...
        file_path = os.path.join(
            "names", line
        )  # returns a file path using the correct seperator.
        open(file_path, "x")  # creates a file with the line name..
